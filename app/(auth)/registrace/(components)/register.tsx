...
import TestComp from "@/app/(auth)/testcomp";
...
export default function Register(){
    ...
    const trackFormStart = useCallback(() => {
        if (!hasStartedForm) {
            const products = [{
                item_id: "predplatne",
                item_name: "predplatne",
                price: 99, // Replace with actual price
                quantity: 1,
                currency: 'CZK'
            }];

            analyticsEvents.beginCheckout(products);
            console.log(products)
            setHasStartedForm(true);
        }
    }, [hasStartedForm, selectedCategory]);

    // Handler for any form input change
                ...
                <div className="mx-auto w-full max-w-lg px-6 md:px-10">
                    <form className="flex flex-col md:flex-row gap-y-10 bg-gray-800 pt-2 py-6 md:py-6 px-4 w-full rounded-md rounded-t-none md:rounded-t-md"
                        ...
                        <div className="w-full">
                            <div className="flex flex-col gap-y-5 w-full">
                                ...
                                <div className="flex flex-col w-full gap-y-3 relative">
                                    ...
                                    <div className="flex flex-row items-around w-full gap-x-2">
                                        <button
                                            className={`w-full ${selectedCategory === "wq8ils5ph00l87u" ? "bg-yellow-500 text-white font-semibold" : "bg-gray-300"} items-center text-center justify-center py-2 rounded-md`}
                                            onClick={() => handleCategorySelect("wq8ils5ph00l87u")}
                                            type="button">
                                            A
                                        </button>
                                        <button
                                            className={`w-full ${selectedCategory === "eysdkjavl07ke2c" ? "bg-yellow-500 text-white font-semibold" : "bg-gray-300"} items-center text-center justify-center py-2 rounded-md`}
                                            onClick={() => handleCategorySelect("eysdkjavl07ke2c")}
                                            type="button">
                                            B
                                        </button>
                                        <button
                                            className={`w-full ${selectedCategory === "e3kh74w2vq5dap2" ? "bg-yellow-500 text-white font-semibold" : "bg-gray-300"} items-center text-center justify-center py-2 rounded-md`}
                                            onClick={() => handleCategorySelect("e3kh74w2vq5dap2")}
                                            type="button">
                                            C
                                        </button>
                                    </div>
                                ...
                                <div className="flex flex-col w-full gap-y-2">
                                    ...
                                    />
                                    {emailFail && <p className="text-red-500">{emailFail}</p>}
                                </div>
                                <div className="flex flex-col sm:flex-row">
                                    <div className="flex flex-col w-full gap-y-2">
                                        ...
                                        />
                                        {nameFail && <p className="text-red-500">{nameFail}</p>}
                                    </div>
                                </div>
                                <div className="flex flex-col w-full gap-y-2">
                                    ...
                                    />
                                    {passwordFail && <p className="text-red-500">{passwordFail}</p>}
                                    {!isPassRegexValid && (
                                ...
                                <div className="flex flex-row w-full gap-x-2 items-center">
                                    ...
                                    />
                                    <label htmlFor="checkboxone" className="text-sm text-gray-300">
                            ...
                            <div className="pt-6">
                                <button
                                    className="w-full py-2 text-center bg-green-400 text-green-700 font-semibold tracking-wide rounded-lg"
                                    type="submit">
                                    Přejít na platbu
                                </button>
                            </div>
