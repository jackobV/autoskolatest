        ...
        <div className=" ">
            <div className="flex flex-col max-w-5xl mx-auto px-4">
                ...
                <div className="flex flex-col md:flex-row gap-x-4 gap-y-4">
                    ...
                    <div className="flex flex-col flex-row bg-white rounded-xl w-full md:w-96 h-fit px-4 pt-4 h-40 w-fit text-sm gap-y-2"><a href={`/otazky/${params.slug}/`}  className="bg-[#F8BC00] text-white w-fit px-2 py-1 rounded-xl">{categoryTitle}</a><a className="bg-[#378F2D] text-white w-fit px-2 py-1 rounded-xl" href={`/otazky/${params.slug}/${next}`}>Další otázka </a><a className="bg-[#AFCA00] text-white w-fit px-2 py-1 rounded-xl" href={`/otazky/${params.slug}/${prev}`}>Předchozí otázka </a></div>
                </div>
            </div>
        </div>)
}